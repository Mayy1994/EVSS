...
import sys
sys.path.append('/home/mybeast/xjj/EVSS/model')

from util_syncbn import _Upsample, SpatialPyramidPooling, SpatialPyramidPooling_spp, SeparableConv2d, _Upsample_sk
...
class ResNet(nn.Module):
    def __init__(self, block, layers, *, num_features=128, k_up=3, efficient=False, use_bn=True,
        ...
        upsamples += [
            _Upsample_sk(num_features, self.inplanes, num_features, use_bn=self.use_bn, k=k_up, use_skip=upsample_skip,
        ...
        upsamples_up2 += [
            _Upsample_sk(num_features, num_features, num_features, use_bn=self.use_bn, k=k_up, use_skip=upsample_skip,
                      only_skip=upsample_only_skip, detach_skip=2 in detach_upsample_skips, fixed_size=target_sizes[0],
                      separable=upsample_separable)]
        upsamples_up2 += [
            _Upsample_sk(num_features, num_features, num_features, use_bn=self.use_bn, k=k_up, use_skip=upsample_skip,
        ...
        self.spp_up1 = SpatialPyramidPooling(num_features, num_levels, bt_size=bt_size, level_size=level_size,
                                         out_size=num_features, grids=spp_grids, square_grid=spp_square_grid,
                                         bn_momentum=0.01 / 2, use_bn=self.use_bn, drop_rate=spp_drop_rate
                                         , fixed_size=target_sizes[3])
        self.spp_up2 = SpatialPyramidPooling(num_features, num_levels, bt_size=bt_size, level_size=level_size,
                                         out_size=num_features, grids=spp_grids, square_grid=spp_square_grid,
                                         bn_momentum=0.01 / 2, use_bn=self.use_bn, drop_rate=spp_drop_rate
                                         , fixed_size=target_sizes[3])
        num_up_remove = max(0, int(log2(output_stride) - 2))
        self.upsample = nn.ModuleList(list(reversed(upsamples[num_up_remove:])))
        self.upsample_up2 = nn.ModuleList(list(upsamples_up2))
        self.upsample_up3 = nn.ModuleList(list(upsamples_up3))


        self.random_init = [self.spp, self.spp_up1, self.spp_up2, self.upsample, self.upsample_up2, self.upsample_up3]
    ...
    def forward_up1(self, features):
        ...
        for skip, up in zip(features[1:], self.upsample):
            x = up(x, skip)
            upsamples += [x]
        upsamples[0] = self.spp_up1.forward(upsamples[0])
        # return x, {'features': features, 'upsamples': upsamples}
        return upsamples

    def forward_up2(self, features):
        x = features[0]
        if self.detach_upsample_in:
            x = x.detach()
        upsamples = []
        for skip, up in zip(features[1:], self.upsample_up2):
            x = up(x, skip)
            upsamples += [x]
        upsamples[0] = self.spp_up2.forward(upsamples[0])
        return upsamples

    def forward_up3(self, features):
        x = features[0]
        if self.detach_upsample_in:
            x = x.detach()
        for skip, up in zip(features[1:], self.upsample_up3):
            features = up(x, skip)
    ...
    def forward(self, image):
        ...
        return self.forward_up3(self.forward_up2(self.forward_up1(self.forward_down(image))))

        # out = self.forward_up2(self.forward_up1(self.forward_down(image)))
        # out = self.forward_up1(self.forward_down(image))
