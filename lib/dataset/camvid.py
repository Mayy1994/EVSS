...
from lib.dataset.utils import transform_im, randomcrop, transforms_im
...
class camvid_video_dataset(Dataset):
    ...
    def get_edge_list(self, list_edge_path):
        self.gt_edge_label_name = []

        with open(list_edge_path, 'r') as f:
            lines = f.readlines()

        for i, line in enumerate(lines):
            _, gt_edge_label_name = line.split()
            for j in range(1, 11):
                self.gt_edge_label_name.append(gt_edge_label_name)
    ...
    def generate_edge(self, label, edge_width=1):
        ...
        label[label==2]=12      # pole
        label[label==9]=12      # pedestrian
        label[label==10]=12     # bicyclist
        label[label!=12]=0
        label[label==12]=1
